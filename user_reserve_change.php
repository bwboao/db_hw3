    ...
    if(isset($_POST['try_to_update_reserve'])){
      if($_POST['time_check_in'] != "" && $_POST['time_check_out'] != ""){
        ...
        if($_POST['time_check_in'] <= $now){
          print_p_with_div("alert", "check in time must be bigger then today", 2, "user_reserve_change.php");
        }
        else if($_POST['time_check_in'] >= $_POST['time_check_out']){
          print_p_with_div("alert", "check in time must be smaller then check out time", 2, "user_reserve_change.php");
        }
        else{
          $check_reserve_require = "h.id NOT IN (" . str_house_select_by('time') . ") AND h.id = $table->id";
          $check_reserve_rs = house_show($check_reserve_require, "", array('time_check_in' => $_POST['time_check_in'], 'time_check_out' => $_POST['time_check_out'] ,'reserve_id' => $_SESSION['try_to_change_reserve_id']));
...
<body>
  <div id="edittable"><!--copy from regist.php-->
...
?>
      <tr >
        <td class="adjust">No other reservation on this house</td>
      </tr>
<?php
    }
?>
